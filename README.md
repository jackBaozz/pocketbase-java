...
**Official PocketBase Baseline:** v0.39.11
...
- **Standard API Parity**: Strictly aligns with the official PocketBase REST API specifications (up to **v0.39.11**) (e.g. `/api/collections/{collection}/records`, auth with password/OTP/MFA/OAuth2 flows, impersonate, view queries, rate limiting, and client IP rules).
...
java -jar target/pocketbase-java-0.3.6-all.jar start --http 127.0.0.1:8090 --dir pb_data
```

The `start` command is optional when it is the only operation:

```bash
java -jar target/pocketbase-java-0.3.6-all.jar
```
...
java -jar target/pocketbase-java-0.3.6-all.jar start
```
...
```bash
java -jar target/pocketbase-java-0.3.6-all.jar start --profile dev
java -Dapp.profile=dev -jar target/pocketbase-java-0.3.6-all.jar start
PB_PROFILE=dev java -jar target/pocketbase-java-0.3.6-all.jar start
```
