...
if command -v mise &> /dev/null; then
    ...
    if mise ls java | grep -q "25-graalvm"; then
        ...
        echo "Command: mise exec java@25-graalvm -- mvn -gs settings.xml -s settings.xml -Pnative clean package -DskipTests $*"
        mise exec java@25-graalvm -- mvn -gs settings.xml -s settings.xml -Pnative clean package -DskipTests "$@"
    else
        ...
        mvn -gs settings.xml -s settings.xml -Pnative clean package -DskipTests "$@"
    fi
else
