...
class SchemaMigrationPlannerTest {
  ...
  void detectsFieldAndIndexChangesWithDestructiveMarkers() {
    ...
    assertTrue(
        plan.stream()
            .anyMatch(
                op -> "renameField".equals(op.get("type")) && "count".equals(op.get("field"))));
    assertTrue(
        plan.stream()
            .anyMatch(
                op -> "addField".equals(op.get("type")) && "published".equals(op.get("field"))));
    assertTrue(
        plan.stream()
            .anyMatch(
                op -> "dropField".equals(op.get("type"))
                    && Boolean.TRUE.equals(op.get("destructive"))));
    assertTrue(
        plan.stream()
            .anyMatch(
                op -> "alterFieldType".equals(op.get("type"))
                    && Boolean.TRUE.equals(op.get("destructive"))));
  ...
  void planHandlesNullOrBlankFieldNamesGracefully() {
    ...
    assertTrue(
        plan.stream()
            .anyMatch(op -> "addField".equals(op.get("type")) && "valid".equals(op.get("field"))));
  ...
  void plannerEmitsDialectSpecificTypeAndIndexOperations() {
    CollectionSchema current =
        collection("posts", new FieldSchema("f1", "count", "text", false, false, false));
    current.indexes = List.of("CREATE INDEX idx_posts_count ON posts (count)");
    CollectionSchema desired =
        collection("posts", new FieldSchema("f1", "count", "number", false, false, false));

    var plan =
        SchemaMigrationPlanner.plan(
            current, desired, name -> "`" + name + "`", JooqDatabase.Engine.MYSQL);

    assertTrue(
        plan.stream()
            .anyMatch(op -> String.valueOf(op.get("sql")).contains("MODIFY COLUMN `count`")));
    assertTrue(
        plan.stream()
            .anyMatch(op -> "DROP INDEX `idx_posts_count` ON `posts`".equals(op.get("sql"))));
  }
