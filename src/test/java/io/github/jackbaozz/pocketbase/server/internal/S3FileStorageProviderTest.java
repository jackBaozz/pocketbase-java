...
class S3FileStorageProviderTest {
    ...
    void providerCanPutGetListStatDeleteAndSignUrlsAgainstS3CompatibleEndpoint() throws Exception {
            ...
            switch (exchange.getRequestMethod()) {
                ...
                case "GET" -> {
                    if (exchange.getRequestURI().getRawQuery() != null
                        ...
                        objects.keySet().forEach(name -> body.append("<Contents><Key>").append(name).append("</Key></Contents>"));
        ...
        S3FileStorageProvider provider = new S3FileStorageProvider(endpoint, "us-east-1", "bucket", "access", "secret", true);

        provider.put("folder/test.txt", new ByteArrayInputStream("hello".getBytes(StandardCharsets.UTF_8)), 5, "text/plain");
        assertEquals("hello", new String(provider.get("folder/test.txt").orElseThrow().readAllBytes(), StandardCharsets.UTF_8));
        assertTrue(provider.list("folder/").contains("folder/test.txt"));
        assertEquals(5, provider.stat("folder/test.txt").orElseThrow().size());
        assertTrue(provider.signedUrl("folder/test.txt", 60).orElseThrow().contains("X-Amz-Signature="));
