...
class FilterToSqlCompilerTest {
  ...
  void parentheses() {
    String sql =
        FilterToSqlCompiler.compile("(title = 'a' OR title = 'b') AND status = 'published'");
    assertTrue(sql.contains("AND"));
    assertTrue(sql.contains("("));
    assertTrue(sql.contains(")"));
  }
  ...
  void dateStringComparisonUsesBinding() {
    var result =
        FilterToSqlCompiler.compileBound(
            "created >= '2026-06-28 10:30:00.000Z'", id -> "\"" + id + "\"");
  ...
  void boundStringDoesNotInlineLiteral() {
    var result =
        FilterToSqlCompiler.compileBound(
            "title = 'Bob\\'s Post' AND count >= 10", id -> "\"" + id + "\"");
  ...
  void containsEscapesLikeWildcards() {
    var result =
        FilterToSqlCompiler.compileBound("title ~ '100%_\\\\match'", id -> "\"" + id + "\"");
  ...
  void requestAuthReference() {
    ...
    var result =
        FilterToSqlCompiler.compileBound("email = @request.auth.email", id -> "\"" + id + "\"");
  ...
  void requestBodyReference() {
    var result =
        FilterToSqlCompiler.compileBound("status = @request.body.status", id -> "\"" + id + "\"");
  ...
  void requestQueryReference() {
    var result =
        FilterToSqlCompiler.compileBound("category = @request.query.cat", id -> "\"" + id + "\"");
  ...
  void requestMethodReference() {
    var result =
        FilterToSqlCompiler.compileBound("@request.method = 'POST'", id -> "\"" + id + "\"");
  ...
  void requestHeadersReference() {
    var result =
        FilterToSqlCompiler.compileBound("host = @request.headers.host", id -> "\"" + id + "\"");
  ...
  void collectionReference() {
    var result =
        FilterToSqlCompiler.compileBound(
            "@collection.users.email = 'test@example.com'", id -> "\"" + id + "\"");
  ...
  void boundParameters() {
    var result =
        FilterToSqlCompiler.compileBound("title = 'hello' AND count > 5", id -> "\"" + id + "\"");
  ...
  void identifierQuoting() {
    var result = FilterToSqlCompiler.compileBound("title = 'hello'", id -> "`" + id + "`");
  ...
  void rejectsTrailingTokens() {
    assertThrows(
        ApiException.class, () -> FilterToSqlCompiler.compile("title = 'hello' status = 'draft'"));
  ...
  void complexFilter() {
    ...
    String sql =
        FilterToSqlCompiler.compile(
            "title ~ 'hello' AND (status = 'published' OR featured = true) AND count > 0");
    assertNotNull(sql);
    assertTrue(sql.contains("AND"));
    assertTrue(sql.contains("OR"));
  }
