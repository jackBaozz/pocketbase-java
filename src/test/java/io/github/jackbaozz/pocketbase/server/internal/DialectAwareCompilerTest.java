...
class DialectAwareCompilerTest {
  ...
  void testJsonExtractMySql() {
    var result =
        FilterToSqlCompiler.compileBound(
            "@request.auth.id = '123'", id -> "`" + id + "`", null, JooqDatabase.Engine.MYSQL);
  ...
  void testJsonExtractPostgres() {
    var result =
        FilterToSqlCompiler.compileBound(
            "@request.auth.id = '123'", id -> "\"" + id + "\"", null, JooqDatabase.Engine.POSTGRES);
  ...
  void testJsonExtractSqlite() {
    var result =
        FilterToSqlCompiler.compileBound(
            "@request.auth.id = '123'", id -> "\"" + id + "\"", null, JooqDatabase.Engine.SQLITE);
    assertTrue(result.sql().contains("json_extract(:request_auth, '$.id')"));
  }
  ...
  void testLikeMySql() {
    var result =
        FilterToSqlCompiler.compileBound(
            "name ~ 'john'", id -> "`" + id + "`", null, JooqDatabase.Engine.MYSQL);
  ...
  void testLikePostgres() {
    var result =
        FilterToSqlCompiler.compileBound(
            "name ~ 'john'", id -> "\"" + id + "\"", null, JooqDatabase.Engine.POSTGRES);
