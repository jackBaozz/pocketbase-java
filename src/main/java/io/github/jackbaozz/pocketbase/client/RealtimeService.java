...
public final class RealtimeService {
  ...
  /**
   * Subscribes to a realtime topic. To subscribe to all records in a collection,
   * use the collection
   * name (e.g. "users"). To subscribe to a specific record, use
        ...
        () -> {
          try {
            ...
            if (response.statusCode() != 200) {
              throw new PocketBaseException(
                  "GET",
                  URI.create("/api/realtime"),
                  new RuntimeException(
                      "Failed to connect to realtime API, status: " + response.statusCode()));
            }
            ...
            initialConnect.completeExceptionally(
                new PocketBaseException("GET", URI.create("/api/realtime"), e));
          } catch (Exception e) {
  ...
  private void submitSubscriptions() {
    ...
    Map<String, Object> body = Map.of("clientId", clientId, "subscriptions",
        new ArrayList<>(subscriptions.keySet()));
