...
public final class CollectionRuleSupport {
    ...
    public static void validate(CollectionSchema collection, String message) {
        validate(collection, message, identifier -> collectionMatches(collection, identifier) ? collection : null);
    }
    ...
    public static void validate(
        ...
        Function<String, CollectionSchema> lookup = identifier -> collectionMatches(collection, identifier)
                ? collection
        ...
        if ("auth".equals(collection.type)) {
            ...
            if (collection.mfa != null && collection.mfa.enabled && collection.mfa.rule != null
    ...
    private static void validateNestedRule(
        ...
        try {
            ...
            if (nested instanceof java.util.Map<?, ?> map && map.containsKey(field)) {
                throw new ApiException(400, message, java.util.Map.of(
                        parent,
                        java.util.Map.of(field, map.get(field))
                ));
            }
            throw e;
        }
    ...
    private static void validateRequestBodyIdentifier(
        ...
        if (bodyField == null) {
            if (!RecordFieldResolverSupport.requestBodyModifierAllowed(null, modifier)) {
                throw invalidRule(message, ruleField, "Unknown or invalid request body field `" + rawPath + "`.");
            }
            return;
        }
        if (dot < 0) {
            if (!RecordFieldResolverSupport.requestBodyModifierAllowed(bodyField, modifier)) {
                throw invalidRule(message, ruleField, "Invalid request body field modifier `" + rawPath + "`.");
            }
            return;
        }
    ...
    private static void validateCollectionIdentifier(
        ...
        if (target == null || !RecordFieldResolverSupport.validPath(collectionLookup, target, targetPath, true)) {
            throw invalidRule(message, ruleField, "Unknown or invalid collection field `" + identifier + "`.");
        }
    }

    private static boolean collectionMatches(CollectionSchema collection, String identifier) {
        return collection != null && (identifier != null)
