...
public final class CollectionResponseSupport {
    private static final List<String> TOKEN_CONFIGS = List.of(
            "authToken",
            "passwordResetToken",
            "emailChangeToken",
            "verificationToken",
    ...
    public static void preserveOAuth2ClientSecrets(
            ...
            CollectionSchema.OAuth2Config updated
    ) {
        if (current == null || current.providers == null || updated == null || updated.providers == null) {
            return;
        }
        Map<String, String> secrets = new LinkedHashMap<>();
        for (CollectionSchema.OAuth2ProviderConfig provider : current.providers) {
            if (provider != null && provider.name != null && provider.clientSecret != null && !provider.clientSecret.isBlank()) {
