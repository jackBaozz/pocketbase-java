...
public final class FilterToSqlCompiler {
  ...
  public static String compile(String filter) {
    return compileInternal(
        filter,
        FilterToSqlCompiler::standardQuoteIdentifier,
        false,
        null,
  ...
  public static CompiledFilter compileBound(
      String filter, Function<String, String> quoteIdentifier) {
    return compileBound(filter, quoteIdentifier, null);
  }

  public static CompiledFilter compileBound(
      String filter, Function<String, String> quoteIdentifier, ContainsRenderer containsRenderer) {
  ...
  private static CompiledFilter compileInternal(
      String filter,
      Function<String, String> quoteIdentifier,
      boolean bindLiterals,
      ContainsRenderer containsRenderer,
      JooqDatabase.Engine engine) {
  ...
  private static class Parser {
      ...
      if (t.type == TokenType.IDENTIFIER) {
        ...
        if (t.value.startsWith("@request.auth.")) {
          return new Operand(
              SqlPart.of(
                  dialectJsonExtract(
                      ":request_auth", t.value.substring("@request.auth.".length()))),
              false,
              null,
              null);
        }
        if (t.value.startsWith("@request.body.")) {
          return new Operand(
              SqlPart.of(
                  dialectJsonExtract(
                      ":request_body", t.value.substring("@request.body.".length()))),
              false,
              null,
              null);
        }
        if (t.value.startsWith("@request.query.")) {
          return new Operand(
              SqlPart.of(
                  dialectJsonExtract(
                      ":request_query", t.value.substring("@request.query.".length()))),
              false,
              null,
              null);
        }
        if (t.value.startsWith("@request.headers.")) {
          return new Operand(
              SqlPart.of(
                  dialectJsonExtract(
                      ":request_headers", t.value.substring("@request.headers.".length()))),
              false,
              null,
              null);
        }
    ...
    private SqlPart buildSqlCondition(Operand left, String op, Operand right) {
      ...
      if (left.isCollection) {
        SqlPart collectionField =
            SqlPart.of(
                escapeIdentifier(left.collectionName) + "." + escapeIdentifier(left.fieldName));
        SqlPart comparison = buildComparison(collectionField, baseOp, right.sql);
        String subquery =
            "SELECT 1 FROM " + escapeIdentifier(left.collectionName) + " WHERE " + comparison.sql();
        return new SqlPart("EXISTS (" + subquery + ")", comparison.bindings());
      }
      if (right.isCollection) {
        SqlPart collectionField =
            SqlPart.of(
                escapeIdentifier(right.collectionName) + "." + escapeIdentifier(right.fieldName));
        SqlPart comparison = buildComparison(collectionField, invertOp(baseOp), left.sql);
        String subquery =
            "SELECT 1 FROM "
                + escapeIdentifier(right.collectionName)
                + " WHERE "
                + comparison.sql();
        return new SqlPart("EXISTS (" + subquery + ")", comparison.bindings());
      }
    ...
    private SqlPart renderContains(SqlPart left, SqlPart right, boolean negated) {
      if (containsRenderer != null) {
        CompiledFilter rendered =
            containsRenderer.render(
