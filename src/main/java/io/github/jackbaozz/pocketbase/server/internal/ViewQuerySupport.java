...
public final class ViewQuerySupport {
    public static final String WILDCARD_ERROR = "wildcard columns (*) are not supported - manually type the collection field names you want the view query to have";
    ...
    public static String normalizeSingleSelect(String query) {
        ...
        if (select.$select().stream().anyMatch(item -> item instanceof Asterisk || item instanceof QualifiedAsterisk)) {
    ...
    public static Map<String, Object> result(List<Column> columns, List<List<Object>> rows) {
        ...
        if (idIndex < 0) {
            throw new IllegalArgumentException("missing required id column (you can use `(ROW_NUMBER() OVER()) as id` if you don't have one)");
        }
        ...
        for (List<Object> values : rows) {
            ...
            for (int columnIndex = 0; columnIndex < columns.size(); columnIndex++) {
                ...
                if (columnIndex == idIndex) {
                    ...
                    if (id.isBlank()) {
                        throw new IllegalArgumentException("the query could return records with empty or invalid ids");
                    }
                    if (!ids.add(id)) {
                        throw new IllegalArgumentException("the query could return records with non-unique ids");
                    }
                    value = id;
                }
    ...
    public static String typeForJavaClass(String name, Class<?> type) {
        ...
        if (Map.class.isAssignableFrom(type) || Iterable.class.isAssignableFrom(type) || type.isArray()) {
            return "json";
        }
        return "text";
    }
