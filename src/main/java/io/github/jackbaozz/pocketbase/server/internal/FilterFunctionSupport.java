...
public final class FilterFunctionSupport {
  ...
  private static final Pattern AMOUNT_MODIFIER =
      Pattern.compile(
          "^([+-])(\\d+(?:\\.\\d+)?)\\s+(seconds?|minutes?|hours?|days?|months?|years?)$",
          Pattern.CASE_INSENSITIVE);
  private static final Pattern CLOCK_MODIFIER =
      Pattern.compile("^([+-])(\\d{1,3}):(\\d{2})(?::(\\d{2}(?:\\.\\d+)?))?$");
  private static final Pattern WEEKDAY_MODIFIER =
      Pattern.compile("^weekday\\s+([0-6])$", Pattern.CASE_INSENSITIVE);

  private FilterFunctionSupport() {
  }

  public static Object evaluate(
      String name, List<Argument> arguments, Function<String, Object> identifierResolver) {
    return switch (name) {
  ...
  private static Object geoDistance(List<Argument> arguments, Function<String, Object> resolver) {
    if (arguments.size() != 4) {
      throw new IllegalArgumentException(
          "[geoDistance] expected 4 arguments, got " + arguments.size());
    }
    ...
    for (int i = 0; i < arguments.size(); i++) {
      ...
      if (!List.of(
          ArgumentType.IDENTIFIER, ArgumentType.NUMBER, ArgumentType.NULL, ArgumentType.BOOLEAN)
          .contains(argument.type())) {
        throw new IllegalArgumentException(
            "[geoDistance] argument " + i + " must be an identifier or number");
      }
  ...
  private static Object strftime(List<Argument> arguments, Function<String, Object> resolver) {
    ...
    if (arguments.size() > 1
      ...
      throw new IllegalArgumentException(
          "[strftime] expects the second argument to be of a valid time-value type");
    }
    ...
    if (timeValue instanceof RuleEvaluator.MultiMatchValues multiMatch) {
      return RuleEvaluator.multiMatch(
          multiMatch.values().stream().map(value -> formatTime(format, value, modifiers)).toList());
    }
    if (timeValue instanceof RuleEvaluator.AnyMatchValues anyMatch) {
      return RuleEvaluator.anyMatch(
          anyMatch.values().stream().map(value -> formatTime(format, value, modifiers)).toList());
    }
    return formatTime(format, timeValue, modifiers);
  }

  private static Double haversine(
      Object lonAValue, Object latAValue, Object lonBValue, Object latBValue) {
  ...
  private static TemporalValue parseTime(Object rawValue, List<String> modifiers) {
    ...
    for (int i = 0; i < modifiers.size(); i++) {
      ...
      if (modifier.isBlank()
          || List.of("unixepoch", "julianday", "auto", "ceiling", "floor").contains(modifier)) {
        continue;
      }
      switch (modifier) {
        ...
        case "start of month" ->
          value = value.withDayOfMonth(1).toLocalDate().atStartOfDay(value.getZone());
        case "start of year" ->
          value = value.withDayOfYear(1).toLocalDate().atStartOfDay(value.getZone());
        default -> {
          ...
          if (amount.matches()) {
            boolean floor =
                i + 1 < modifiers.size() && "floor".equalsIgnoreCase(modifiers.get(i + 1).trim());
  ...
  private static ZonedDateTime numericTime(double value, String mode) {
    boolean unixEpoch =
        "unixepoch".equals(mode) || "auto".equals(mode) && (value < 0D || value > 5373484.499999D);
    if (unixEpoch) {
  ...
  private static ZonedDateTime shiftMonths(ZonedDateTime value, double amount, boolean floor) {
    ...
    ZonedDateTime shifted =
        ZonedDateTime.of(target.atDay(targetDay), value.toLocalTime(), value.getZone());
  ...
  private static ZonedDateTime applyClock(ZonedDateTime value, Matcher matcher) {
    double seconds =
        Integer.parseInt(matcher.group(2)) * 3600D
  ...
  private static String formatToken(char token, ZonedDateTime value, boolean subseconds) {
    return switch (token) {
      ...
      case 'F' ->
        pad(value.getYear(), 4)
            + "-"
            + pad(value.getMonthValue(), 2)
            + "-"
            + pad(value.getDayOfMonth(), 2);
      case 'R' -> pad(value.getHour(), 2) + ":" + pad(value.getMinute(), 2);
      case 'T' ->
        pad(value.getHour(), 2)
            + ":"
            + pad(value.getMinute(), 2)
            + ":"
            + pad(value.getSecond(), 2);
  ...
  private static String julianDay(ZonedDateTime value) {
    ...
    return String.format(Locale.ROOT, "%.9f", julian)
        .replaceFirst("0+$", "")
        .replaceFirst("\\.$", "");
  }
