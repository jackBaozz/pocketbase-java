...
public final class PasswordHasher {
  ...
  public static boolean verifyOrDummy(String password, String encoded) {
    ParsedHash parsed = parse(encoded);
    boolean matched =
        verifyParsed(password == null ? "" : password, parsed == null ? DUMMY_HASH : parsed);
  ...
  private static ParsedHash parse(String encoded) {
    ...
    try {
      return new ParsedHash(Integer.parseInt(parts[1]), decode(parts[2]), decode(parts[3]));
    } catch (RuntimeException e) {
      return null;
    }
  }
  ...
  private static ParsedHash createDummyHash() {
    ...
    return new ParsedHash(
        ITERATIONS, salt, pbkdf2("pocketbase-java-dummy".toCharArray(), salt, ITERATIONS));
  }
