...
public final class AuthCollectionConfigMerge {
    ...
    public static void mergeSubmitted(
            ObjectMapper mapper,
            CollectionSchema target,
            CollectionSchema base,
            JsonNode body
    ) {
        ...
        if (passwordAuth != null) {
            target.passwordAuth = merge(mapper, defaults.passwordAuth, passwordAuth, CollectionSchema.PasswordAuthConfig.class);
        }
        ...
        if (authAlert != null) {
            target.authAlert = merge(mapper, defaults.authAlert, authAlert, CollectionSchema.AuthAlertConfig.class);
        }
        target.authToken = mergeToken(mapper, target.authToken, defaults.authToken, option(body, "authToken"));
        target.passwordResetToken = mergeToken(mapper, target.passwordResetToken, defaults.passwordResetToken, option(body, "passwordResetToken"));
        target.verificationToken = mergeToken(mapper, target.verificationToken, defaults.verificationToken, option(body, "verificationToken"));
        target.emailChangeToken = mergeToken(mapper, target.emailChangeToken, defaults.emailChangeToken, option(body, "emailChangeToken"));
        target.fileToken = mergeToken(mapper, target.fileToken, defaults.fileToken, option(body, "fileToken"));
        JsonNode verificationTemplate = option(body, "verificationTemplate");
        if (verificationTemplate != null) {
            target.verificationTemplate = merge(mapper, defaults.verificationTemplate, verificationTemplate, CollectionSchema.EmailTemplate.class);
        }
        JsonNode resetPasswordTemplate = option(body, "resetPasswordTemplate");
        if (resetPasswordTemplate != null) {
            target.resetPasswordTemplate = merge(mapper, defaults.resetPasswordTemplate, resetPasswordTemplate, CollectionSchema.EmailTemplate.class);
        }
        JsonNode confirmEmailChangeTemplate = option(body, "confirmEmailChangeTemplate");
        if (confirmEmailChangeTemplate != null) {
            target.confirmEmailChangeTemplate = merge(mapper, defaults.confirmEmailChangeTemplate, confirmEmailChangeTemplate, CollectionSchema.EmailTemplate.class);
        }
    }
    ...
    private static CollectionSchema.TokenConfig mergeToken(
        ...
        CollectionSchema.TokenConfig merged = merge(mapper, base, patch, CollectionSchema.TokenConfig.class);
