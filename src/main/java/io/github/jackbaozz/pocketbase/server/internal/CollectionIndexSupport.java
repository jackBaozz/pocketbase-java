...
public final class CollectionIndexSupport {
  ...
  private static final Set<String> SQL_WORDS =
      Set.of(
          "and",
          "or",
          "not",
  ...
  public static List<String> normalize(
      CollectionSchema collection, Collection<String> externalIndexNames, String message) {
    ...
    for (int i = 0; i < rawIndexes.size(); i++) {
      ...
      if (parsed == null) {
        throw indexError(
            message, i, "validation_invalid_index_expression", "Invalid CREATE INDEX expression.");
      }
      String lowerName = parsed.name().toLowerCase(Locale.ROOT);
      if (!names.add(lowerName)) {
        throw indexError(
            message, i, "validation_duplicated_index_name", "The index name already exists.");
      }
      if (externalNames.contains(lowerName)) {
        throw indexError(
            ...
            "The index name is already used in another collection.");
      }
  ...
  public static String createSql(
      String raw, String table, Function<String, String> quoteIdentifier) {
    ParsedIndex parsed = parse(raw);
    return parsed == null ? "" : build(parsed, table, quoteIdentifier);
  }

  /**
   * Produces executable DDL for a specific relational engine while retaining the unmodified
   * PocketBase index definition in collection metadata.
   */
  public static String createSql(
      String raw,
      String table,
  ...
  private static String build(
      ParsedIndex index, String table, Function<String, String> quoteIdentifier) {
  ...
  private static String build(
    ...
    sql.append(quoteIdentifier.apply(index.name()))
        ...
        .append(" (");
    Map<String, Integer> mysqlPrefixes =
        engine == JooqDatabase.Engine.MYSQL ? mysqlPrefixLengths(index, fields) : Map.of();
    String mysqlPartialCondition =
        engine == JooqDatabase.Engine.MYSQL && !index.where().isBlank()
            ? rewriteQuotedIdentifiers(index.where(), quoteIdentifier)
            : "";
    List<String> columns = new ArrayList<>();
    for (IndexColumn column : index.columns()) {
      String expression =
          engine == JooqDatabase.Engine.MYSQL
              ? mysqlIndexExpression(
                  index, column, quoteIdentifier, mysqlPrefixes, mysqlPartialCondition)
  ...
  private static String mysqlIndexExpression(
    ...
    if (!partialCondition.isBlank()) {
      ...
      return mysqlFunctionalKeyPart(
          "CASE WHEN " + partialCondition + " THEN " + expression + " ELSE NULL END");
    }
  ...
  private static Map<String, Integer> mysqlPrefixLengths(
      ParsedIndex index, List<FieldSchema> fields) {
    ...
    for (IndexColumn column : index.columns()) {
      ...
      MysqlStringColumn stringColumn =
          mysqlStringColumn(name, fieldsByName.get(name.toLowerCase(Locale.ROOT)));
    ...
    boolean needsPrefix =
        totalCharacters > 768 || strings.stream().anyMatch(MysqlStringColumn::forcePrefix);
    if (!needsPrefix) {
      return Map.of();
    }
    ...
    for (int i = 0; i < strings.size(); i++) {
      ...
      int allocation =
          Math.min(column.maxCharacters(), Math.max(1, remainingCharacters / remainingColumns));
  ...
  private static MysqlStringColumn mysqlStringColumn(String name, FieldSchema field) {
    ...
    return switch (type) {
      case "text", "editor" ->
        new MysqlStringColumn(
            name, "tokenKey".equals(field.name) && field.system ? 255 : 2000, false);
      ...
      case "select", "json", "file", "relation", "geopoint" ->
        new MysqlStringColumn(name, 768, true);
      default -> null;
    };
  }

  private static String normalizedColumnExpression(
      String raw, Function<String, String> quoteIdentifier) {
  ...
  private static String rewriteQuotedIdentifiers(
      String expression, Function<String, String> quoteIdentifier) {
    ...
    for (Pattern pattern : List.of(
      ...
      while (matcher.find()) {
        matcher.appendReplacement(
            normalized, Matcher.quoteReplacement(quoteIdentifier.apply(matcher.group(1))));
      }
  ...
  private static String definitionKey(ParsedIndex index) {
    ...
    for (IndexColumn column : index.columns()) {
      columns.add(
          (column.expression() + "|" + column.collate() + "|" + column.sort())
    ...
    return index.unique()
        ...
        + String.join(",", columns)
        + "|"
        + index.where().toLowerCase(Locale.ROOT);
  }

  private static ApiException indexError(
      String message, int index, String code, String errorMessage) {
