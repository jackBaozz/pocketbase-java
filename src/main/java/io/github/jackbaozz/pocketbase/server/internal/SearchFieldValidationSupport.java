...
public final class SearchFieldValidationSupport {
    ...
    private static final Set<String> RECORD_SYSTEM_FIELDS = Set.of("id", "created", "updated");
    private static final Set<String> STATIC_FILTER_VALUES = Set.of("@now", "@todayStart", "@todayEnd");
    ...
    public static void validateRecords(
        ...
        validateFilter(search.filter(), field -> validRecordField(store, collection, field, allowHidden));
        for (SearchQuerySupport.SortTerm term : search.sort()) {
            if (!term.random() && !term.rowId() && !validRecordField(store, collection, term.name(), allowHidden)) {
                throw invalidSearchParameters();
            }
        }
    }

    private static void validateFilter(String filter, java.util.function.Predicate<String> validator) {
