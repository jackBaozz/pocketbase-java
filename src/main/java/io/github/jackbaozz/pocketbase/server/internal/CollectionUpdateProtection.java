...
public final class CollectionUpdateProtection {
  ...
  public static void validate(
      CollectionSchema current, CollectionSchema updated, JsonNode body, String message) {
  ...
  static void collectErrors(
    ...
    if (!errors.containsKey("type")
        && body.has("type")
        && !Objects.equals(current.type, requestedType)) {
      errors.put(
          "type",
          ApiErrors.validationError(
              "validation_collection_type_change", "Collection type cannot be changed."));
    }
    if (!errors.containsKey("system")
        && body.has("system")
        && body.path("system").asBoolean() != current.system) {
      errors.put(
          "system",
          ApiErrors.validationError(
  ...
  private static void compareRule(
      JsonNode body, Map<String, Object> errors, String field, String current, String updated) {
    ...
    for (int i = 0; i < updated.fields.size(); i++) {
      ...
      if (previous != null && !Objects.equals(previous.type, next.type)) {
        fieldErrors.put(
            String.valueOf(i),
            ApiErrors.validationError(
                "validation_field_type_change", "Field type cannot be changed."));
      }
    }
