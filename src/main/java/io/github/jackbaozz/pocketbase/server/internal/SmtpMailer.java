...
public final class SmtpMailer {
  ...
  public record Message(
      String fromName, String fromAddress, String to, String subject, String html, String text) {
    String raw() {
    ...
    void authenticate(Settings settings) throws IOException {
      String method =
          settings.authMethod() == null
              ? ""
              : settings.authMethod().trim().toUpperCase(Locale.ROOT);
      if ("LOGIN".equals(method)) {
        command("AUTH LOGIN", 334);
        command(
            Base64.getEncoder()
                .encodeToString(settings.username().getBytes(StandardCharsets.UTF_8)),
            334);
        command(
            Base64.getEncoder()
                .encodeToString(settings.password().getBytes(StandardCharsets.UTF_8)),
            235);
        return;
      }
