...
public final class FieldValidator {
  ...
  private static Object normalizeEmail(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
    String email = value.asText().trim().toLowerCase(Locale.ROOT);
    if (!email.contains("@") || email.startsWith("@") || email.endsWith("@")) {
      errors.put(
          field.name,
          ApiErrors.validationError("validation_invalid_email", "Must be a valid email address."));
      return email;
    }

    if (field.options != null) {
      if (field.options.containsKey("exceptDomains")
          && field.options.get("exceptDomains").isArray()) {
          ...
          if (email.endsWith("@" + domain) || email.endsWith("." + domain)) {
            errors.put(
                ...
                ApiErrors.validationError(
                    "validation_invalid_email", "Email domain is not allowed."));
            break;
          }
        }
      }
      if (field.options.containsKey("onlyDomains")
          && field.options.get("onlyDomains").isArray()
          && !field.options.get("onlyDomains").isEmpty()) {
  ...
  private static Object normalizePassword(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
      ...
      if (field.options.containsKey("min")) {
        ...
        if (pwd.length() < min) {
          errors.put(
              field.name,
              ApiErrors.validationError(
                  "validation_length_out_of_range", "Cannot be less than " + min + " characters."));
        }
      }
      if (field.options.containsKey("max")) {
        ...
        if (pwd.length() > max) {
          errors.put(
              field.name,
              ApiErrors.validationError(
                  "validation_length_out_of_range", "Cannot be more than " + max + " characters."));
        }
      }
      if (field.options.containsKey("pattern")
          && !field.options.get("pattern").asText().isEmpty()) {
        String pattern = field.options.get("pattern").asText();
        try {
          if (!Pattern.compile(pattern).matcher(pwd).matches()) {
            errors.put(
                field.name,
                ApiErrors.validationError("validation_invalid_format", "Invalid password format."));
          }
        } catch (Exception e) {
  ...
  private static Object normalizeBoolean(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
    if (value.isBoolean()) {
    ...
    if (value.isNumber()) {
      ...
      errors.put(
          field.name,
          ApiErrors.validationError("validation_invalid_format", "Expected boolean value."));
      return null;
    }
  ...
  private static Object normalizeNumber(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
    Double numVal = null;
    if (value.isNumber()) {
      ...
      try {
        ...
        errors.put(
            field.name,
            ApiErrors.validationError("validation_invalid_format", "Expected numeric value."));
        return null;
      }
    } else {
      errors.put(
      ...
      if (field.options.containsKey("min")) {
        ...
        if (numVal < min) {
          errors.put(
              field.name,
              ApiErrors.validationError(
                  "validation_invalid_value", "Cannot be less than " + min + "."));
        }
      }
      if (field.options.containsKey("max")) {
        ...
        if (numVal > max) {
          errors.put(
              field.name,
              ApiErrors.validationError(
                  "validation_invalid_value", "Cannot be more than " + max + "."));
        }
      }
    }
  ...
  private static Object normalizeSelect(
      ObjectMapper mapper, FieldSchema field, JsonNode value, Map<String, Object> errors) {
        ...
        for (JsonNode item : value) {
          if (!allowed.contains(item.asText())) {
            errors.put(
                field.name,
                ApiErrors.validationError(
                    "validation_invalid_value", "Value is not in the allowed list."));
          }
        }
        int maxSelect =
            field.options.containsKey("maxSelect") ? field.options.get("maxSelect").asInt(1) : 1;
        if (value.size() > maxSelect && maxSelect > 0) {
          errors.put(
              field.name,
              ApiErrors.validationError("validation_invalid_value", "Too many values selected."));
        }
  ...
  private static Object normalizeUrl(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
    ...
    try {
      ...
      if (uri.getScheme() == null
        ...
        errors.put(
            field.name,
            ApiErrors.validationError("validation_invalid_url", "Must be a valid HTTP/HTTPS URL."));
        return urlText;
      }
      if (field.options != null) {
        JsonNode allowedHosts =
            field.options.containsKey("onlyHosts")
                ? field.options.get("onlyHosts")
                : field.options.get("onlyDomains");
        if (allowedHosts != null && allowedHosts.isArray() && !allowedHosts.isEmpty()) {
          ...
          if (!matched) {
            errors.put(
                field.name,
                ApiErrors.validationError("validation_invalid_url", "URL host is not allowed."));
          }
        }
        JsonNode exceptHosts =
            field.options.containsKey("exceptHosts")
                ? field.options.get("exceptHosts")
                : field.options.get("exceptDomains");
            ...
            for (JsonNode hostNode : exceptHosts) {
              if (host.equalsIgnoreCase(hostNode.asText())) {
                errors.put(
                    field.name,
                    ApiErrors.validationError(
                        "validation_invalid_url", "URL host is not allowed."));
                break;
              }
            }
          }
      ...
      errors.put(
          field.name, ApiErrors.validationError("validation_invalid_url", "Must be a valid URL."));
    }
    return urlText;
  }

  private static Object normalizeText(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
      ...
      if (field.options.containsKey("pattern")
        ...
        try {
          if (!Pattern.compile(pattern).matcher(text).matches()) {
            errors.put(
                field.name,
                ApiErrors.validationError("validation_invalid_format", "Invalid format."));
          }
        } catch (Exception e) {
  ...
  private static Object normalizeDate(
      FieldSchema field, JsonNode value, Map<String, Object> errors) {
        ...
        if (field.options.containsKey("min") && !field.options.get("min").asText().isEmpty()) {
          ...
          if (dateText.compareTo(minDate) < 0) {
            errors.put(
                field.name,
                ApiErrors.validationError(
                    "validation_invalid_date", "Cannot be before " + minDate + "."));
          }
        }
        if (field.options.containsKey("max") && !field.options.get("max").asText().isEmpty()) {
          ...
          if (dateText.compareTo(maxDate) > 0) {
            errors.put(
                field.name,
                ApiErrors.validationError(
                    "validation_invalid_date", "Cannot be after " + maxDate + "."));
          }
        }
      }
    } catch (Exception e) {
      errors.put(
          field.name, ApiErrors.validationError("validation_invalid_date", "Invalid date format."));
    }
    return dateText;
  }

  private static Object normalizeRelation(
      ObjectMapper mapper,
      FieldSchema field,
      JsonNode value,
      Map<String, Object> errors,
        ...
        if (value.isArray()) {
          for (JsonNode item : value) {
            if (!recordExists.test(targetCollection, item.asText())) {
              errors.put(
                  field.name,
                  ApiErrors.validationError(
                      "validation_missing_record", "Failed to resolve relation " + item.asText()));
            }
          }
          int maxSelect =
              field.options.containsKey("maxSelect") ? field.options.get("maxSelect").asInt(1) : 1;
          ...
          if (!recordExists.test(targetCollection, value.asText())) {
            errors.put(
                field.name,
                ApiErrors.validationError(
                    "validation_missing_record", "Failed to resolve relation " + value.asText()));
          }
        }
      }
    }
