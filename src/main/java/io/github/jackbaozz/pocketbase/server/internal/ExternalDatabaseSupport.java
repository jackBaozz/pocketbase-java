...
public final class ExternalDatabaseSupport {
    ...
    Value url =
        firstValue(
            List.of(
                ...
                new Candidate(
                    "system property " + prefix + ".url", propertyReader.apply(prefix + ".url")),
                new Candidate("environment PB_DATABASE_URL", envReader.apply("PB_DATABASE_URL")),
                new Candidate(
                    "environment PB_" + envPrefix + "_URL",
    ...
    Value user =
        firstValue(
            List.of(
                ...
                new Candidate(
                    "system property " + prefix + ".user", propertyReader.apply(prefix + ".user")),
                new Candidate("environment PB_DATABASE_USER", envReader.apply("PB_DATABASE_USER")),
                new Candidate(
                    "environment PB_" + envPrefix + "_USER",
    ...
    Value password =
        firstValue(
            List.of(
                ...
                new Candidate(
                    "system property " + prefix + ".password",
                    propertyReader.apply(prefix + ".password")),
                new Candidate(
                    "environment PB_DATABASE_PASSWORD", envReader.apply("PB_DATABASE_PASSWORD")),
                new Candidate(
                    "environment PB_" + envPrefix + "_PASSWORD",
  ...
  public static String missingUrlMessage(String storage) {
    ...
    return "Missing JDBC URL for "
        + prefix
        + " storage. Set -Ddb.url, -D"
        + prefix
        + ".url, PB_DATABASE_URL, or PB_"
        + envPrefix
        + "_URL.";
  }
  ...
  public record ResolvedConfig(
      String storage, String url, String user, String password, String source) {
