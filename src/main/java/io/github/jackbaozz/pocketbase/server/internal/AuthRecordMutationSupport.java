...
public final class AuthRecordMutationSupport {
  ...
  public static boolean hasManageAccess(
    ...
    return hasManageAccess(
        collection, record, body, RuleRequestContext.of(query, Map.of()), method, principal, store);
  }
  ...
  public static void validate(
    ...
    if (!create && !manageAccess) {
      if (body.has("email")
          && !Objects.equals(text(existing.get("email")), body.path("email").asText())) {
        errors.put(
            "email",
            ApiErrors.validationError(
                "validation_values_mismatch",
                "The email address can be changed only by a manager."));
      }
      if (body.has("verified")
          && truthy(existing.get("verified")) != body.path("verified").asBoolean()) {
        errors.put(
            "verified",
            ApiErrors.validationError(
                "validation_values_mismatch",
                "The verified state can be changed only by a manager."));
      }
    }

    boolean passwordMutation =
        body.has(passwordField) || body.has("passwordConfirm") || body.has("oldPassword");
    if (passwordMutation) {
      String password = body.path(passwordField).asText("");
      String passwordConfirm = body.path("passwordConfirm").asText("");
      if (password.isBlank()) {
        errors.put(
            passwordField, ApiErrors.validationError("validation_required", "Cannot be blank."));
      }
      if (passwordConfirm.isBlank()) {
        errors.put(
            "passwordConfirm",
            ApiErrors.validationError("validation_required", "Cannot be blank."));
      } else if (!password.equals(passwordConfirm)) {
        errors.put(
            "passwordConfirm",
            ApiErrors.validationError("validation_values_mismatch", "Passwords do not match."));
      }
      if (!create && !manageAccess) {
        ...
        if (oldPassword.isBlank()
          ...
          errors.put(
              "oldPassword",
              ApiErrors.validationError(
                  "validation_invalid_old_password", "Missing or invalid old password."));
        }
      }
    }
