...
public final class RuleEvaluator {
  ...
  public static Context context(
    ...
    return context(
        record, body, query, Map.of(), method, principal, collectionResolver, pathResolver);
  }
    ...
    while (index < expression.length()) {
      ...
      while (index < expression.length()) {
        ...
        if (Character.isWhitespace(current)
            ...
            || current == ')'
            || current == ','
            || startsOperator(expression, index)) {
          break;
        }
        index++;
      }
  ...
  private static boolean compare(Object left, String operator, Object right) {
    ...
    if (left instanceof MultiMatchValues multiMatch) {
      ...
      return any
          ...
          : multiMatch.values().stream()
              .allMatch(value -> compareOne(value, actualOperator, right));
    }
  ...
  private static boolean valuesEqual(Object left, Object right) {
    ...
    if (right instanceof Collection<?> collection) {
      return collection.contains(left)
          || collection.stream().anyMatch(item -> valuesEqual(left, item));
    }
  ...
  public record Context(
    ...
    public Context {
      ...
      requestContext =
          requestContext == null || requestContext.isBlank()
              ? RuleRequestContext.DEFAULT
              : requestContext;
    ...
    Object resolve(String identifier) {
      ...
      Object value =
          switch (normalized) {
            ...
            case "@todayStart" ->
              LocalDate.now(ZoneOffset.UTC).atStartOfDay().toInstant(ZoneOffset.UTC).toString();
            case "@todayEnd" ->
            ...
            case "@request.auth.collectionId" ->
              principal != null ? principal.collectionId() : null;
            case "@request.auth.collectionName" ->
              principal != null ? principal.collectionName() : null;
    ...
    private Object resolvePath(String identifier) {
      ...
      if (identifier.startsWith("@request.auth.")) {
        return read(
            principal == null ? Map.of() : principal.asRuleMap(),
      ...
      if (identifier.startsWith("@request.headers.")) {
        return read(
            headers, identifier.substring("@request.headers.".length()).toLowerCase(Locale.ROOT));
      }
    ...
    private boolean pathExists(String identifier) {
      ...
      if (identifier.startsWith("@request.auth.")) {
        return principal != null
            && containsPath(principal.asRuleMap(), identifier.substring("@request.auth.".length()));
      }
      if (identifier.startsWith("@request.headers.")) {
        return headers.containsKey(
            identifier.substring("@request.headers.".length()).toLowerCase(Locale.ROOT));
      }
    ...
    private Object parseFunction(String name) {
      ...
      if (peek().type != TokenType.RPAREN) {
        do {
          ...
          FilterFunctionSupport.ArgumentType type =
              switch (argument.type) {
                ...
                default ->
                  throw invalidFilter("Invalid argument for filter function `" + name + "`.");
              };
          Object value =
              switch (argument.type) {
                case NUMBER ->
