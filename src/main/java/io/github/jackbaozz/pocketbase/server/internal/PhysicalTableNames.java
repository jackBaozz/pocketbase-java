...
/**
 * Maps API-visible collection names to identifiers accepted by every supported
 * relational database. PocketBase allows collection names up to 255 characters,
 * while PostgreSQL and MySQL limit physical identifiers to 63 and 64 bytes.
 */
public final class PhysicalTableNames {
    ...
    public static String tableName(JooqDatabase database, String logicalName) {
        ...
        return TABLE_PREFIX + sha256(logicalName).substring(0, EXTERNAL_IDENTIFIER_LIMIT - TABLE_PREFIX.length());
    }
    ...
    public static String primaryKeyName(JooqDatabase database, String tableName) {
        ...
        return CONSTRAINT_PREFIX + sha256(candidate).substring(0, EXTERNAL_IDENTIFIER_LIMIT - CONSTRAINT_PREFIX.length());
    }
